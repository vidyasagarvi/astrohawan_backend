...
<p>Dear <b>${order.data.user_name}</b>,</p>
...
<div style="margin: 20px 0; border: 1px solid #ddd; border-radius: 5px; padding: 10px; background-color: #f9f9f9;">
  ...
  <p><strong>Order Date:</strong> ${order.data.order_date}</p>
  <p><strong>Delivery Address::</strong> ${order.data.shipping_address}</p>
  <p><strong>Total Amount:</strong> ₹${order.data.total_ammount} (including GST ₹${order.data.gst_charge} and shipping ₹${order.data.shipping_charge})</p>
</div>
...
<p style="font-weight: bold;">${order.data.shipping_address}, ${order.data.pin_code}</p>
...
<p style="margin: 0; font-size: 11px;">&copy; 2024 Santim. All rights reserved.</p>
    ...
    `
        ...
        const mailOptions2 = {
            ...
            subject: `Your order from ${order.data.user_name}`,
            html: `
       <div style="font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: auto; padding: 0; border-radius: 10px; border: 1px solid #ddd; background-color: #ffffff;">
  <!-- Header -->
  <div style="background-color: #5B1D06; padding: 10px; border-radius: 10px 10px 0 0;">
...
<!-- Content -->
 ...
 <div style="padding: 20px; font-size: 14px; line-height: 1.6;">
    ...
    <p><strong>Order ID:</strong> ${order.data.id}</p>
    <p><strong>User Name:</strong> ${order.data.user_name}</p>
    <p><strong>User Email:</strong> ${order.data.user_email}</p>
    <p><strong>User Phone:</strong> ${order.data.user_phone}</p>
    <p><strong>Shipping Address:</strong> ${order.data.shipping_address}</p>
    <p><strong>Total Amount:</strong> ₹${order.data.total_ammount}</p>
    <p><strong>Order Date:</strong> ${order.data.order_date}</p>
    <p><strong>Order Status:</strong> ${order.data.order_status}</p>
    ...
    <p><strong>GST Charge:</strong> ₹${order.data.gst_charge}</p>
    <p><strong>Shipping Charge:</strong> ₹${order.data.shipping_charge}</p>
